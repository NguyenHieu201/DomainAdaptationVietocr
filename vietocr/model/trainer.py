...
class Trainer():
    def __init__(self, config, pretrained=True, augmentor=ImgAugTransform()):
        ...
        self.source_annotation = config['dataset']['source_annotation']
        self.dataset_name = config['dataset']['name']

        self.batch_size = config['trainer']['batch_size']
        self.print_every = config['trainer']['print_every']
        self.valid_every = config['trainer']['valid_every']
        self.big_iter = config['trainer']['big_iter']
...
#            **config['optimizer'])
        ...
        self.source_gen = self.data_gen('source_{}'.format(self.dataset_name),
                                        self.data_root, self.source_annotation, masked_language_model=False)
    ...
    def train(self):
        ...
        source_iter = iter(self.source_gen)
        for i in range(self.num_iters):
            ...
            start = time.time()
            # normal training
            loss = self.step(batch, is_discriminator_training=False)

            # discriminator training
            self.step(batch, is_discriminator_training=True, is_source=False)
            self.step(source_batch, is_discriminator_training=True,
                      is_source=True)
...
#                loss = self.criterion(rearrange(outputs, 'b t v -> (b t) v'), rearrange(tgt_output, 'b o -> (b o)'))
    ...
    def step(self, batch, is_discriminator_training=False, is_source=False):
        self.model.train()
        ...
        if is_discriminator_training:
            # print("Not use discriminator")
            # discriminator_target = None
            num_sample = img.shape[0]
            # print(img.shape)
            outputs = self.model(
                img, tgt_input, tgt_key_padding_mask=tgt_padding_mask, discriminator=True)
            discriminator_target = torch.zeros((num_sample, 1))
            if not is_source:
                discriminator_target = torch.zeros((num_sample, 1)) + 1
            discriminator_target = discriminator_target.to(self.device)
            loss = self.discriminator_loss(outputs, discriminator_target)
